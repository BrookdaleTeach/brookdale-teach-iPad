...
@implementation WritingAssessmentModel

+ (NSString *) getDatabasePath {
    return kWriting_Database;
} /* getDatabasePath */
...
+ (NSMutableArray *) getAllInsertSchemas {
    ...
    [array addObject:kInsert_writing_mechanics];
    [array addObject:kInsert_writing_organization];
    [array addObject:kInsert_writing_process];
...
+ (void) insertStudentDataIntoClassDatabase :(NSString *)uid {
    ...
    for ( int x = 0; x < [self getNumberOfSections]; x++ ) {
        ...
        if (x == 0) {
            table = kTable_writing_mechanics;
            insert = kInsert_writing_mechanics;
            array = [[kObjects_writing_mechanics componentsSeparatedByString:@","] copy];
        } else if (x == 1) {
            table = kTable_writing_organization;
            insert = kInsert_writing_organization;
            array = [[kObjects_writing_organization componentsSeparatedByString:@","] copy];
        } else {
            table = kTable_writing_process;
            insert = kInsert_writing_process;
            array = [[kObjects_writing_process componentsSeparatedByString:@","] copy];
...
+ (void) insertDataIntoClassDatabase :(NSString *)uid section :(int)s row :(int)r text :(NSString *)text {
    ...
    if (s == 0) {
        table = kTable_writing_mechanics;
        parsedArray = [[kObjects_writing_mechanics componentsSeparatedByString:@","] mutableCopy];
        object = [[parsedArray objectAtIndex:r] stringByReplacingOccurrencesOfString:@" " withString:@""];
    } else if (s == 1) {
        table = kTable_writing_organization;
        parsedArray = [[kObjects_writing_organization componentsSeparatedByString:@","] mutableCopy];
        object = [[parsedArray objectAtIndex:r] stringByReplacingOccurrencesOfString:@" " withString:@""];
    } else {
        table = kTable_writing_process;
        parsedArray = [[kObjects_writing_process componentsSeparatedByString:@","] mutableCopy];
        object = [[parsedArray objectAtIndex:r] stringByReplacingOccurrencesOfString:@" " withString:@""];
    }
    ...
    if (sqlite3_open([[self checkAndCreateDatabase] UTF8String], &database) == SQLITE_OK) {
        ...
        if (sqlite3_step(compiledStatement) != SQLITE_DONE) {
            NSLog(@"Save Error ([Writing]Insert Student): %s", sqlite3_errmsg(database) );
        }
...
+ (NSString *) checkAndCreateDatabase {
    ...
    if (true) {
        if (success) {
            ...
            NSLog(@"Writing SQL database not found. If this is a new installation, \
    ...
    for ( int x = 0; x < [self getNumberOfSections]; x++ ) {
        ...
        if (sqlite3_open([[self checkAndCreateDatabase] UTF8String], &database) == SQLITE_OK) {
            ...
            if (sqlite3_prepare(database, sqlStatement, -1, &compiledStatement, NULL) != SQLITE_OK) {
                NSLog(@"[Writing]Problem with prepare statement: %@", [NSString stringWithUTF8String:(char *)sqlite3_errmsg(database)]);
            }
            ...
            if (sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) == SQLITE_OK) {
                ...
                while (sqlite3_step(compiledStatement) == SQLITE_ROW) {
                    ...
                    while ( x < sqlite3_column_count(compiledStatement) - 1) {
                        NSLog(@"[Writing]RETRIEVE: %@", [NSString stringWithUTF8String:(char *)sqlite3_column_text(compiledStatement, x)]);
...
+ (void) insertFormativeDataIntoClassDatabase :(NSString *)uid :(NSMutableDictionary *)dictionary {
    ...
    NSString *insertData = [NSString stringWithFormat:@"INSERT INTO %@(uid,%@) VALUES(\"%@\",\"%@\")", kTable_writing_formative, kColumn_formative, uid, data];
    if (sqlite3_open([[self checkAndCreateDatabase] UTF8String], &database) == SQLITE_OK) {
...
+ (void) insertStandardizedDataIntoClassDatabase :(NSString *)uid :(NSMutableDictionary *)dictionary {
    ...
    NSString *insertData = [NSString stringWithFormat:@"INSERT INTO %@(uid,%@) VALUES(\"%@\",\"%@\")", kTable_writing_standard, kColumn_standard, uid, data];
    if (sqlite3_open([[self checkAndCreateDatabase] UTF8String], &database) == SQLITE_OK) {
        const char *sqlChar = [insertData UTF8String];
        sqlite3_stmt *compiledStatement;
...
+ (NSMutableArray *) selectFormativeDataIntoClassDatabase :(NSString *)uid {
    ...
    NSString *selectStatement = [NSString stringWithFormat:@"SELECT %@ FROM %@ WHERE uid='%@'", kColumn_formative, kTable_writing_formative, uid];
...
+ (NSMutableArray *) selectStandardizedDataIntoClassDatabase :(NSString *)uid {
    ...
    NSString *selectStatement = [NSString stringWithFormat:@"SELECT %@ FROM %@ WHERE uid='%@'", kColumn_standard, kTable_writing_standard, uid];
