...
typedef enum {
    TableViewCellPositionTop = 0,
    TableViewCellPositionMiddle,
    TableViewCellPositionBottom,
    TableViewCellPositionAlone
} TableViewCellPosition;



/** `TableViewCell` is a subclass of `UITableViewCell`, so it
 is fully compatible with any `UITableView`.
 
 `TableViewCell` adds a set of customizations such as corner radius, 
 ...
 references to `UITableViewCell` to `TableViewCell`, and add this call:
 ...
 tableView:neededHeightForIndexPath: to your desired height.
    ...
    For example:
        ...
        {
            return tableView.rowHeight + [TableViewCell 
 ...
 Cells are compatible with both grouped and plain tables. 
...
/** Sets the cell's position to help the background drawing. 
 ...
 Possible values are:
 - `TableViewCellPositionTop`
 - `TableViewCellPositionMiddle`
 - `TableViewCellPositionBottom`
 - `TableViewCellPositionAlone`
 */
@property (nonatomic, assign) TableViewCellPosition position;
