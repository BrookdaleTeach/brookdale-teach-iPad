...
@implementation MathTestTableViewController

- (id) initWithStyle :(UITableViewStyle)style :(Student *)st {
    self = [super initWithStyle:UITableViewStyleGrouped];
    if (self) {
        // Custom initialization
        self.title = NSLocalizedString(@"Math Standardized Tests/Formative Assessments", nil);
        student = st;
    }
    return self;
} /* initWithStyle */
...
- (void) viewDidLoad {
    ...
    standardizedTests = [[NSMutableArray alloc] initWithArray:[MathAssessmentModel selectStandardizedDataIntoClassDatabase:[student uid]]];
    formativeAssessments = [[NSMutableArray alloc] initWithArray:[MathAssessmentModel selectFormativeDataIntoClassDatabase:[student uid]]];

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(reloadData:)
                                                 name:@"ReloadMathTestTableView"
                                               object:nil];
} /* viewDidLoad */
...
- (void) tableView :(UITableView *)tableView commitEditingStyle :(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath :(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        if (indexPath.section == 0)
            [MathAssessmentModel deleteFromTestAssesments:[student uid] :[standardizedTests objectAtIndex:indexPath.row] :kStandardized_Key];
        else if (indexPath.section == 1)
            [MathAssessmentModel deleteFromTestAssesments:[student uid] :[formativeAssessments objectAtIndex:indexPath.row] :kFormative_Key];
...
- (void) tableView :(UITableView *)tableView didSelectRowAtIndexPath :(NSIndexPath *)indexPath {
    ...
    if ((indexPath.row == standardizedTests.count) && (indexPath.section == 0)) {
        NewStandardizedTest *nst = [[NewStandardizedTest alloc] initWithStyle:UITableViewStyleGrouped :student :1 :kEditingMode_NewEntity :nil];
        UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController:nst];
        navController.modalPresentationStyle = UIModalPresentationFormSheet;
        [self presentModalViewController:navController animated:YES];

    } else if ((indexPath.row == formativeAssessments.count) && (indexPath.section == 1)) {
        NewFormativeAssessment *nfa = [[NewFormativeAssessment alloc] initWithStyle:UITableViewStyleGrouped :student :1 :kEditingMode_NewEntity :nil];
        ...
        NewStandardizedTest *nst = [[NewStandardizedTest alloc] initWithStyle:UITableViewStyleGrouped :student :1 :kEditingMode_EntityExists :indexPath];
        UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController:nst];
        navController.modalPresentationStyle = UIModalPresentationFormSheet;
        [self presentModalViewController:navController animated:YES];
    } else if ((indexPath.row < formativeAssessments.count) && (indexPath.section == 1)) {
        NewFormativeAssessment *nst = [[NewFormativeAssessment alloc] initWithStyle:UITableViewStyleGrouped :student :1 :kEditingMode_EntityExists :indexPath];
        UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController:nst];
        navController.modalPresentationStyle = UIModalPresentationFormSheet;
        [self presentModalViewController:navController animated:YES];
    }
