//
//  WritingTestTableViewController.m
//  Bobcats
//
...
@implementation WritingTestTableViewController

- (id) initWithStyle :(UITableViewStyle)style :(Student *)st {
    self = [super initWithStyle:UITableViewStyleGrouped];
    if (self) {
        // Custom initialization
        self.title = NSLocalizedString(@"Writing Standardized Tests/Formative Assessments", nil);
        student = st;
    }
    return self;
} /* initWithStyle */
...
- (void) viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(reloadData:)
                                                 name:@"ReloadWritingTestTableView"
                                               object:nil];
} /* viewDidLoad */
...
- (void) tableView :(UITableView *)tableView didSelectRowAtIndexPath :(NSIndexPath *)indexPath {
    ...
    if ((indexPath.row == standardizedTests.count) && (indexPath.section == 0)) {
        NewStandardizedTest *nst = [[NewStandardizedTest alloc] initWithStyle:UITableViewStyleGrouped :student :3];
        ...
        NewFormativeAssessment *nfa = [[NewFormativeAssessment alloc] initWithStyle:UITableViewStyleGrouped :student :3];
